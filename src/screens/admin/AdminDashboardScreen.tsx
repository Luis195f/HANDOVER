...
import type { IceaOpsErrorSummary, IceaOpsEventSummary, IceaOpsUnitSummary } from '../../types/admin';
...
function ErrorCard({ item }: { item: IceaOpsErrorSummary }) {
  return (
    ...
    >
      <Text style={{ fontWeight: '600' }}>{item.source} · {item.errorFamily}</Text>
...
function UnitCard({
  ...
  return (
    ...
    >
      ...
      <Text>Outbox queued/retry/failed: {unit.counts.outbox.queued}/{unit.counts.outbox.retry}/{unit.counts.outbox.failed}</Text>
      <Text>Bridge pending/scored/stale: {unit.counts.bridge.pending}/{unit.counts.bridge.scored}/{unit.counts.bridge.stale}</Text>
      <Text>Pipeline running/retry/failed: {unit.counts.pipeline.running}/{unit.counts.pipeline.retry}/{unit.counts.pipeline.failed}</Text>
...
export function AdminDashboardScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 16 }}>
      ...
      {summary.state && summary.available ? (
        <Banner state={summary.state}>Estado global: {stateLabel(summary.state)}</Banner>
      ) : null}
      ...
      {isEmpty ? (
        <View style={{ paddingVertical: 24 }}>
          ...
          <Text style={{ color: '#64748b' }}>Cuando entren handovers, outbox, bridge o eventos de pipeline, esta vista dejará de estar vacía.</Text>
        </View>
      ) : null}
