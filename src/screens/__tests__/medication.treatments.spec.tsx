...
function renderWithForm<T extends { control: unknown }>(
  ...
  function Wrapper() {
    const methods = useForm<HandoverFormValues>({ defaultValues: initialValues });
...
describe('MedicationSection', () => {
  ...
  it('muestra campos de horario al activar infusión continua', async () => {
    const { getByPlaceholderText, getByTestId, getByText } = renderWithForm(
      MedicationSection,
      defaultValues,
    );
    ...
    // Nota: este testID debe existir en el Switch de "infusión continua" dentro de MedicationSection.
    // Si tu componente aún no lo tiene, añádelo como: testID="medication.continuousInfusion"
    fireEvent(getByTestId('medication.continuousInfusion'), 'valueChange', true);
