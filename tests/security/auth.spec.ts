...
beforeEach(async () => {
  // 0) Limpiar mocks de vitest (sin resetear módulos)
  vi.clearAllMocks();
  ...
  globalAny.fetch = vi.fn(async () => ({
    ...
    json: async () => ({
      ...
      // ✅ Claim real usado por tu app (según logs/jwt.io)
      "https://handover/roles": ["nurse"],

      // ✅ Mantén esto por compatibilidad si alguna parte todavía lo usa
      roles: ["nurse"],

      // Si tu app extrae unidades del userinfo, mantenemos esto igual
      units: ["icu-a"],
    }),
  }));
});
