...
| Auditoría (hash sin PHI + `patientKey` canónico `ptk2_*` en persistencia/lectura, compat móvil `ptk_` en transporte, rechazo de raw IDs en `/api/audit` y logs estructurados sin `meta`) | `backend/audit/service.py`, `backend/audit/views.py`, `backend/api/audit_pseudonymization.py`, `backend/api/views.py`, `backend/api/views_ai.py`, `backend/api/migrations/0014_pseudonymize_client_audit_patient_ids.py`, `backend/api/migrations/0015_sanitize_client_audit_event_meta.py`, `backend/api/migrations/0016_rotate_client_audit_patient_keys_v2.py`, `src/lib/audit.ts`, `src/screens/AuditLogScreen.tsx` | `backend/audit/tests/test_audit.py`, `backend/api/tests/test_audit_log_models.py`, `tests/audit.spec.ts`, `tests/screens/AuditLogScreen.spec.tsx` | `docs/SECURITY_HARDENING.md`, `docs/security-and-auth.md` |
| Dataset sintético demo | `demo/fhir-bundles/*.json` | N/A (validación manual) | `docs/MVP_DEMO.md` |
| IA SBAR segura + guardrails | `backend/ai_client.py`, `backend/api/views_ai.py` | `backend/tests/test_ai_prompting.py` | `docs/SECURITY_HARDENING.md` |
