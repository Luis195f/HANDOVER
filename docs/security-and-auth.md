...
- `context` y `handover` comparten la allowlist recursiva: `dxMedical` (string, máximo 240 caracteres), `dxNursing` (string sin límite individual), `evolution` (string, máximo 4 000), `vitals` y `oxygenTherapy`. `evolution` se admite para compatibilidad, pero el DTO mínimo del cliente no la envía por defecto.
- `vitals` admite únicamente `hr`, `rr`, `tempC`, `spo2`, `sbp`, `dbp`, `glucoseMgDl`, `glucoseMmolL` (números acotados por el serializer) y `avpu` (`A`, `C`, `V`, `P` o `U`). `oxygenTherapy` admite `device` (string sin límite individual), `flowLMin` y `fio2` (números acotados). El único límite de estos textos antes del egress es el prompt compuesto de 15 000 caracteres; si se supera, responde `400` con `code=ai_prompt_too_large` y una única auditoría fallida censurada.
